<template>
  <div>
    <div class="toolbar" v-show="false">
...
export default {
  ...
  methods: {
    ...
    handlePreviewChange() {
      ...
      this.$router.push(
        `/designlistread/${this.$route.query.project_id}?share_code=${this.$route.query.share_code}`
      );
    },
    importFile() {
      if (this.selectedItem === "A") {
        this.$store.commit("setComponentData", pageList[0].canvasData);
        this.$store.commit("setCanvasStyle", pageList[0].canvasStyle);
